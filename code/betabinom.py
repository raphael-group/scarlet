...
scalefactor = 1.1579208923731619542357098500869e+77  # 2**256  ' used for rescaling calcs w/o impacting accuracy, to avoid over/underflow
...
lfbArray = [cf_0, cf_1, cf_2, cf_3, cf_4, cf_5, cf_6, cf_7, cf_8, cf_9, cf_10, cf_11, cf_12, cf_13, cf_14, cf_15, cf_16, cf_17, cf_18, cf_19, cf_20, cf_21, cf_22, cf_23, cf_24, cf_25, cf_26, cf_27, cf_28, cf_29]
coeffs=[0.32246703342411321824,6.7352301053198095133e-02,2.0580808427784547879e-02,7.3855510286739852663e-03,2.8905103307415232858e-03,1.1927539117032609771e-03,5.0966952474304242234e-04,2.2315475845357937976e-04,\
       9.9457512781808533715e-05,4.4926236738133141700e-05,2.0507212775670691553e-05,9.4394882752683959040e-06,4.3748667899074878042e-06,2.0392157538013662368e-06,9.5514121304074198329e-07,4.4924691987645660433e-07,\
       2.1207184805554665869e-07,1.0043224823968099609e-07,4.7698101693639805658e-08,2.2711094608943164910e-08,1.0838659214896954091e-08,5.1834750419700466551e-09,2.4836745438024783172e-09,1.1921401405860912074e-09,\
       5.7313672416788620133e-10,2.7595228851242331452e-10,1.3304764374244489481e-10,6.4229645638381000221e-11,3.1044247747322272762e-11,1.5021384080754142171e-11,7.2759744802390796625e-12,3.5277424765759150836e-12,\
       1.7119917905596179086e-12,8.3153858414202848198e-13,4.0422005252894400655e-13,1.9664756310966164904e-13,9.5736303878385557638e-14,4.6640760264283742246e-14,2.2737369600659723206e-14,1.1091399470834522017e-14,\
       5.4136591567253631315e-15,2.6438800178609949985e-15,1.2918959062789967293811764562316e-15,6.3159355041984485676779394847024e-16,1.421085482803160676983430580383e-14]
coeffs2=[9.96703342411321823620758332301e-3,4.85230105319809513323333333333e-3,2.35164176111788121233425746862e-3,1.1355510286739852662730972914e-3,5.4676033074152328575298829848e-4,2.6269438789373716759012616902e-4,\
        1.2601997117161385090708338501e-4,6.03943417869127130948e-5,2.89279988929196448257e-5,1.38537935563149598820e-5,6.63558635521614609862e-6,3.17947224962737026300e-6,1.52432377823166362836e-6,\
        7.31319548444223379639e-7,3.51147479316783649952e-7,1.68754473874618369035e-7,8.11753732546888155551e-8,3.90847775936649142159e-8,1.88369052760822398681e-8,9.08717580313959348175e-9,4.38794008336117216467e-9,\
        2.12078578473653627963e-9,1.02595225309999020577e-9,4.96752618206533915776e-10,2.40726205228207715756e-10,1.16751848407213311847e-10,5.66693373586358102002e-11,2.75272781658498271913e-11,\
        1.33812334011666457419e-11,6.50929603319331702812e-12,3.16857905287746826547e-12,1.54339039998043529180e-12,7.52239805801019839357e-13,3.66855576849641617566e-13,1.79011840661361776213e-13,\
        8.73989502840846835258e-14,4.26932273880725309600e-14,2.08656067727432658676e-14,1.02026669800712891581e-14,4.99113012967463749398e-15,2.44274876330334144841e-15,1.19604099925791156327e-15,\
        5.85859784064943690566e-16,2.87087981566462948467e-16,1.40735520163755175636e-16]
...
lfbc6 = 691.0 / 30030.0
lfbc7 = 1.0 / 13.0
lfbc8 = .35068485511628418514 #Chosen to make logfbit(6) & logfbit(7) correct
...
# Accurate calculation of log(1+x)-x, particularly for small x.
   if (abs(x) < 0.01):
      term = x / (2.0 + x)
      y = term * term
      return term * ((((2.0 / 9.0 * y + 2.0 / 7.0) * y + 0.4) * y + 2.0 / 3.0) * y - x)
  ...
  #    return 3.316287351993628748511050974106e-02   #  3.316287351993628748511050974106e-02
    ...
    if x <= 0.0:
       ...
       for i in range(i-1,0,-1):
          ...
          #print(i,lgam)
       return ((coeffs0Minusp25 - (x * (1.0 / 3.0 - (x + 1.5) * logcf(-x, 4.0, 1.0)) + lgam) * x) * x + HalfMinusEulers_const) * x + lfb_0
    elif x <= 1.56:
       x = x - 1.0
       i = len(coeffs2) + 2
       ...
       for i in range(i-1,0,-1):
          lgam = (coeffs[i] - x * lgam)
          #print(lgam)
       return ((x * x * (coeffs0Minus1Third - x * lgam) - (x + 2.5) * log1(x / 3.0) + log1(x / 2.0)) + FiveOver3Minusln3Minuseulers_const * x) + lfb_2
    else:
       x = x - 3.0
       i = len(coeffs)-1
       lgam = coeffs[i] * logcf(-x / 2.0, i + 2.0, 1.0)
       ...
       for i in range(i-1,0,-1):
          lgam = (coeffs[i] - x * lgam)
          #print(lgam)
       return ((x * x * (coeffs0Minusp25 - x * lgam) - (x + 3.5) * log1(x / 4.0) + log1(x / 2.0) + log1(x / 3.0)) + Forty7Over48Minusln4Minuseulers_const * x) + lfb_3
...
# Calculates logfbit(b)-logfbit(a+b) accurately for a > 0 & b >= 0. Reasonably accurate for a >=0 & b < 0.
  if a < 0.0:
    ...
    for i in range(i-3, -1, -1):
       ...
       y2 = (coeffs2[i] - y1 * y2)
       x2 = (coeffs2[i] - x1 * x2)
    return s1 + (y1 * y1 * acc - a * (x2 * (x1 + y1) + Onep25Minusln2Minuseulers_const))
  else:
    raise ValueError
...
# Parameterised this way for use with the Beta Negative Binomial distribution.
   ...
   if ((ai == 0.0) and ((aji <= 0.0) or (aki <= 0.0) or (amj < 0.0) or (amk < 0.0))):
     ...
     return math.exp((c1 + 1.0 / am1) + c3) * math.sqrt((amk1 * ak1) * (aj1 * amj1) / ((amkji1 * aki1 * aji1) * (am1 * ai1))) * OneOverSqrTwoPi
   else:
     return 0.0
...
# Parameterised this way for use with the Beta Negative Binomial distribution.
     ...
     if (amkji > -1.0) and (amkji < 0.0):
        ...
        ip1 = amkji + 1.0
        mkji = amkji
        allintegral = ai == math.floor(ai) and aji == math.floor(aji) and aki == math.floor(aki) and mkji == math.floor(mkji)
     ...
     if ((i >= 0.0) and (ji <= 0.0) or (ki <= 0.0) or (ip1 + ki <= 0.0) or (ip1 + ji <= 0.0)):
        exact = True
        if (i >= 0.0):
           prob = 1.0
        else:
     ...
     if (maxSums > sumAlways or must_do_cf):
        numb = math.floor(sumFactor / c4 * math.exp(math.log((ki + i) * (ji + i) * (ip1 + ji) * (ip1 + ki)) / 3.0))
...
def PBB(i, ssmi, beta_shape1, beta_shape2):
   ...
   return (beta_shape1 / (i + beta_shape1)) * (beta_shape2 / (beta_shape1 + beta_shape2)) * ((i + ssmi + beta_shape1 + beta_shape2) / (ssmi + beta_shape2)) * hTerm
...
def pmf_BetaNegativeBinomial(i,r, beta_shape1, beta_shape2):
   ...
   if (r <= 0.0 or beta_shape1 <= 0.0 or beta_shape2 <= 0.0):
      ...
      return (beta_shape2 / (beta_shape1 + beta_shape2)) * (r / (beta_shape1 + r)) * beta_shape1 * (i + beta_shape1 + r + beta_shape2) / ((i + r) * (i + beta_shape2)) * hypergeometricTerm(i, r, beta_shape2, beta_shape1)
...
# Assumption here that i is integral or greater than 4.
      ...
      return hypergeometric(i, r, beta_shape2, beta_shape1 - 1.0, False)#, ha1, hprob, hswap)
...
def CBNB2(i,r, beta_shape1, beta_shape2):
   ...
   if ss + d1 > bs2: d1 = math.floor(bs2 - ss)
   ss = ss + d1
   j = i - d1
...
def ccBNB5(ilim , rr , a , bb):
   ...
   return (r * b * (1.0 - temp) - math.expm1(ccbnb5) * a * (r + a + b)) / ((r + a) * (a + b))
...
def sf_BetaNegativeBinomial(i,r, beta_shape1, beta_shape2):
   ...
   if (other <= 0.0 or beta_shape1 <= 0.0 or beta_shape2 <= 0.0):
      ...
      return 1.0
   elif (i == 0.0) or ((i < 1000000.0) and (other < 0.001) and (beta_shape1 > 50.0 * other) and (10.0 * i * beta_shape1 < mrb2)):
      return ccBNB5(i, mrb2, beta_shape1, other)
   elif (mrb2 >= 100.0 or other > 20.0 or (mrb2 >= 5.0 and (other - 0.5) * (mrb2 - 0.5) > (i + 0.5) * (beta_shape1 + 0.5))):
...
# These probably should be checked once, when creating a BetaBinomial distribution object
   if ((beta_shape1 <= 0.0) or (beta_shape2 <= 0.0) or (sample_size < 0.0)):
      ...
      return (beta_shape1 / (i + beta_shape1)) * (beta_shape2 / (beta_shape1 + beta_shape2)) * ((sample_size + beta_shape1 + beta_shape2) / (sample_size - i + beta_shape2)) * hypergeometricTerm(i, sample_size - i, beta_shape1, beta_shape2)
...
def pmf_hypergeometric(type1s, sample_size, tot_type1, pop_size):
   ...
   if ((sample_size < 0.0) or (tot_type1 < 0.0) or (sample_size > pop_size) or (tot_type1 > pop_size)):
      raise ValueError
   else:
      return hypergeometricTerm(type1s, sample_size - type1s, tot_type1 - type1s, pop_size - tot_type1 - sample_size + type1s)
...
def cdf_hypergeometric(type1s, sample_size, tot_type1, pop_size):
   ...
   if ((sample_size < 0.0) or (tot_type1 < 0.0) or (sample_size > pop_size) or (tot_type1 > pop_size)):
      ...
      return hypergeometric(type1s, sample_size - type1s, tot_type1 - type1s, pop_size - tot_type1 - sample_size + type1s, False)
...
def sf_hypergeometric(type1s, sample_size, tot_type1, pop_size):
   ...
   if ((sample_size < 0.0) or (tot_type1 < 0.0) or (sample_size > pop_size) or (tot_type1 > pop_size)):
      ...
      return hypergeometric(type1s, sample_size - type1s, tot_type1 - type1s, pop_size - tot_type1 - sample_size + type1s, True)
...
def pmf_neghypergeometric(type2s, type1s_reqd, tot_type1, pop_size):
   ...
   if ((type1s_reqd <= 0.0) or (tot_type1 < type1s_reqd) or (tot_type1 > pop_size)):
      ...
      return hypergeometricTerm(type1s_reqd - 1.0, type2s, tot_type1 - type1s_reqd + 1.0, pop_size - tot_type1 - type2s) * (tot_type1 - type1s_reqd + 1.0) / (pop_size - type1s_reqd - type2s + 1.0)
...
def cdf_neghypergeometric(type2s, type1s_reqd, tot_type1, pop_size):
   ...
   if ((type1s_reqd <= 0.0) or (tot_type1 < type1s_reqd) or (tot_type1 > pop_size)):
      ...
      return hypergeometric(type2s, type1s_reqd, pop_size - tot_type1 - type2s, tot_type1 - type1s_reqd, False)
...
def sf_neghypergeometric(type2s, type1s_reqd, tot_type1, pop_size):
   ...
   if ((type1s_reqd <= 0.0) or (tot_type1 < type1s_reqd) or (tot_type1 > pop_size)):
      ...
      return hypergeometric(type2s, type1s_reqd, pop_size - tot_type1 - type2s, tot_type1 - type1s_reqd, True)
